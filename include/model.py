...
class Graph2HeuristicModel(nn.Module):
    ...
    def forward(self, risk, start, destination, targets = None):
        ...
        des_emb = des_emb.unsqueeze(1).expand(risk_emb.size(0), T, pos_emb.size(1))#(B, T, C)
        start_emb = start_emb.unsqueeze(1).expand(risk_emb.size(0), T, pos_emb.size(1))#(B, T, C)
        #print(des_emb.shape)
        risk_emb = risk_emb.view(risk_emb.size(0), T, pos_emb.size(1))#(B, T, C)
        #print(risk_emb.shape)
        #x = pos_emb + start_emb + des_emb + risk_emb#(B, T, C)
        x = start_emb + des_emb + risk_emb #try no pos_emb
